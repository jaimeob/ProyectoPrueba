...
class NewOrderLines extends Component {
  constructor(props) {
    super(props);
    this.state = {
      products: [],
  ...
  render() {
    ...
    return (
      <div>
        <Typography variant="body1" className={classes.titleStep}>{Utils.messages.NewOrder.title}</Typography>
        <Autocomplete
          resource="products"
          param="name"
        ...
        />
        {
          ...
          <div className={classes.container}>
            <Empty
              title="¡Sin productos!"
              description="No has ingresado productos a la solicitud"
            />
          </div>
          ...
          <Paper className={classes.container}>
            <Table>
              <TableHead>
                <TableRow>
                  <TableCell><strong>{Utils.messages.NewOrder.titleList}</strong></TableCell>
              ...
              <TableBody>
                {this.state.products.map((item, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell component="th" scope="item">
                      $ {Utils.numberWithCommas(item.data.priceList.toFixed(2))}
                    </TableCell>
