...
class Warehouses extends Component {
  ...
  createWarehouse(event) {
    this.props.history.push(Utils.constants.paths.createWarehouse)
  }
        ...
        <div>
          <Grid container>
            <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
              <Title
                title="Almacenes."
                description="Módulo de gestión y control de almacenes."
              />
            </Grid>
            <Grid item xl={12} lg={12} md={12} sm={12} xs={12} style={{ marginTop: 8, marginBottom: 8 }}>
              <Head
                searchPlaceholder="Buscar por nombre o sucursal..."
                titleButtonCreate="Nuevo almacén"
                searchQuery={ (data) => { this.searchQuery(data) } }
                callToCreate={ (module.permissions.create) ? ( (event) => { this.createWarehouse(event) } ) : false }
              />
            </Grid>
          </Grid>
          {
            (!this.state.loading) ?
              ...
              <Empty 
                title="¡Sin almacenes!"
                description="No hay almacenes para mostrar."
              />
            :
            <Empty 
