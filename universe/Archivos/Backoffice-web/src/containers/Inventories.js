...
class Inventories extends Component {
  ...
  createInventory(event) {
    this.props.history.push(Utils.constants.paths.createInventory)
  }
        ...
        <div>
          <Grid container>
            <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
              <Title
                title="Inventarios."
                description="Consulta los inventarios por sucursal y almacén."
              />
            </Grid>
            <Grid item xl={12} lg={12} md={12} sm={12} xs={12} style={{ marginTop: 8, marginBottom: 8 }}>
              <Head
                searchPlaceholder="Buscar por nombre..."
                titleButtonCreate="Nuevo inventario"
                searchQuery={ (data) => { this.searchQuery(data) } }
                callToCreate={ (module.permissions.create) ? ( (event) => { this.createInventory(event) } ) : false }
              />
            </Grid>
          </Grid>
          {
            (!this.state.loading) ?
              ...
              <Empty 
                ...
                description="No hay inventarios para mostrar."
              />
            :
            <Empty 
