...
class AddressFormNew extends Component {
  constructor(props) {
    super(props)
    this.state = {
      openSnack: false,
              ...
              </Grid>
              {
                (this.state.locations.length > 0 || this.props.editAddress) ?
                  <div>
                    ...
                    {
                      (true) ?
                        <div style={{ marginTop: 16, width: '100%', height: '400px', position: 'relative' }}>
                          <GoogleMapReact
                            style={{ marginTop: 16, width: '100%', height: '100%', position: 'relative' }}
                            bootstrapURLKeys={{ key: 'AIzaSyAyTOd_cml2QodRI_Z9FDhV1HYdTmiSqPo' }}
                            center={{
                              lat: (this.state.lat !== null && this.state.lat !== '' && this.state.lat !== 0) ? this.state.lat : 24.806553,
                              lng: (this.state.lng !== null && this.state.lng !== '' && this.state.lng !== 0) ? this.state.lng : -107.393753
                            }}
                            zoom={15}
                            onClick={(event) => {
                              this.setState({
                          ...
                          >
                            <div lat={(this.state.lat !== null && this.state.lat !== '' && this.state.lat !== 0) ? this.state.lat : 24.806553} lng={(this.state.lng !== null && this.state.lng !== '' && this.state.lng !== 0) ? this.state.lng : -107.393753}><img style={{ marginTop: -20, marginLeft: -20, width: 50, height: 50 }} src="/pin.svg" /></div>
                          </GoogleMapReact>
                        </div>
                        :
                        ''
...
export default compose(withStyles(styles), connect(mapStateToProps, null))(AddressFormNew)
