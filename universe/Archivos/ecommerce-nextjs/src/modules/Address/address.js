          ...
          <Hidden mdUp>
            <Grid item xs={12} style={{ paddingLeft: 16 }}>
              ...
              <Link href={`/mi-cuenta/[pid]`} as={`/mi-cuenta/${Utils.constants.paths.addresses}`} color="inherit" style={{ display: 'inline-block' }}>
                <Typography style={{ color: "#283A78", fontSize: 18 }}>Mis direcciones</Typography>
              </Link>
            </Grid>
          </Hidden>
          <Grid item className={classes.container} xs={12} sm={12} md={8} style={{ margin: '0 auto' }}>
            <Typography variant="h2" className={classes.title}><heavy>Mis direcciones</heavy></Typography>
            <Typography className={classes.txt}>Administra tus direcciones</Typography>
            <MyAddresses />
          </Grid>
        </Grid>
      </div>
