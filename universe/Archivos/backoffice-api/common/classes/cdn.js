...
const upload = async (upload, model, where, data) => {
  return new Promise(async (resolve, reject) => {
    let document = await Utils.createUniqueMongoDB(model, where, data)
    if (document.success) {
      let aws = await uploadFile(upload)
      if (aws.success) {
        await Utils.findAndUpdateMongoDB('CDN', where, { '$set': { url: aws.data.Location } })
...
module.exports = ({ listBuckets, uploadFile, upload, createFileIntoCDN })
